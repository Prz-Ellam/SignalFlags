			...
			<form @submit.prevent="createUser" class="p-5 col-lg-5 col-md-9 shadow-sm" novalidate>
				...
				<div class="form-group text-center mb-4">
					...
					<input type="file" accept="image/png, image/gif, image/jpeg, image/jpg"
						class="form-control position-absolute" name="profile-picture" id="profile-picture"
						autocomplete="off" @change="uploadProfilePicture">
					<label for="profile-picture" role="button">Foto de perfil</label>
				</div>

				<input type="hidden" name="" v-model="profilePicture">
				<small class="text-danger" v-if="v$.profilePicture.$dirty && v$.profilePicture.required.$invalid">La foto de
				...
				<div class="mb-4">
					...
					<ul class="mt-2">
						<li
							:class="(password !== '') ? (v$.password.containsUpper.$invalid) ? 'text-danger' : 'text-success' : ''">
							Contiene mayúsculas</li>
						<li
							:class="(password !== '') ? (v$.password.containsLower.$invalid) ? 'text-danger' : 'text-success' : ''">
							Contiene minúsculas</li>
						<li
							:class="(password !== '') ? (v$.password.containsNumber.$invalid) ? 'text-danger' : 'text-success' : ''">
							Contiene números</li>
						<li
							:class="(password !== '') ? (v$.password.containsSpecialChars.$invalid) ? 'text-danger' : 'text-success' : ''">
							Contiene carácteres especiales</li>
						<li
							:class="(password !== '') ? (v$.password.minLength.$invalid) ? 'text-danger' : 'text-success' : ''">
